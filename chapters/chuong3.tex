...
Từ các phân tích trên, phương pháp thiết kế được chọn cho đề tài là: kiến trúc đa board phân lớp chức năng, giao tiếp dữ liệu theo khung có kiểm lỗi, điều khiển phân cấp OBC--EXP. Cách lựa chọn này cân bằng giữa tính khả thi triển khai, độ tin cậy truyền dữ liệu và mục tiêu nghiên cứu của luận văn.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{picture/exp/block-exp-detail-1.png}\\[1em]
    \includegraphics[width=0.95\textwidth]{picture/exp/block-exp-detail-2.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{picture/if/block-if-detail.png}
...
\begin{itemize}
    ...
    \item \textbf{Khối SWITCHING $6 \times 6$:} thực hiện ma trận chọn kênh phát, định tuyến dòng kích từ mạch nguồn dòng đến từng phần tử laser theo lệnh điều khiển.

    \item \textbf{Khối LASER $6 \times 6$:} là ma trận phần tử phát quang đầu ra, phát ánh sáng theo kênh đã được chọn để kích thích mẫu trong buồng đo.
\end{itemize}
...
IC TPS5430DDAR có dãy điện áp ngõ vào rộng từ 5V5 đến 36V, cho phép dòng chảy liên tục tối đa lên đến 3A và dòng đỉnh tối đa 4A. Hiệu suất chuyển đổi của IC có thể đạt tới 95\% trong điều kiện hoạt động bình thường với điện trở nội Rds(on) trên mosfet là 100m $\Omega$. Ramp nội bộ 500 kHz làm chuẩn thời gian cho băm xung đầu ra từ việc dùng error voltage từ op-amp (khi so sánh VSENSE với điện áp tham chiếu) so sánh với ramp để tạo duty cycle. IC TPS5430DDAR có thể điều chỉnh điện áp ngõ ra dựa vào tổ hợp 2 giá trị điện trở ở cầu phân áp từ ngõ ra về chân VSENSE theo công thức bên dưới.
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối đo nguồn 12V và 5V\_FPGA}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối SPI switch LVDS TX}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{picture/if/block-can.png}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối LASER - sw-int}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối LASER - sw-ext}
...
Tầng thứ ba sử dụng bộ lọc Sallen-Key. Bộ lọc Sallen-key là một trong những cấu trúc phổ biến nhất để thiết kế bộ lọc tích cực bậc hai. Trong mạch thiết kế, tầng Sallen-Key được sử dụng ở giai đoạn cuối cùng trước khi đưa tín hiệu vào ADC nhằm thực hiện chức năng Anti-aliasing (chống chồng lấn phổ). Ưu điểm của Sallen-Key là bộ lọc bậc 2, cung cấp độ dốc lên tới $40\text{ dB/decade}$ so với bộ lọc RC đơn giản chỉ có độ dốc $20\text{ dB/decade}$. Nó giúp triệt tiêu các nhiễu tần số cao (nhiễu nguồn, nhiễu EMI trên vệ tinh) nhanh hơn và mạnh hơn, đảm bảo vùng Nyquist của ADC hoàn toàn sạch nhiễu. Theo như thiết kế trên board PHOTO, tần số lấy mẫu $f_s$ = 500KHz nên $f_{Nyquist}$ = 250KHz.
