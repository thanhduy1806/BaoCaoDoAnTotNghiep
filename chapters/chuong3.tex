...
\begin{figure}[H]
    ...
    \caption{Schematic khối USB host only}
...
\end{figure}
Theo schematic của bo EXP, khối MPD200A là phần đại diện cho module nguồn HV mà MCU tương tác thông qua các tín hiệu \texttt{DAC}, \texttt{EN} và \texttt{PWM}; trong đó \texttt{EN} dùng để cho phép/khóa hoạt động, còn \texttt{DAC}/\texttt{PWM} dùng để đặt mức hoặc điều chế công suất theo yêu cầu điều khiển. Ngõ ra của khối là cặp cao áp \texttt{HV\_O+} và \texttt{HV\_O-} đưa ra connector tới tải.

Đối chiếu với datasheet MPD-200, các chân điều khiển chuẩn của module chủ yếu ở mức hệ nguồn (remote on/off, power good, remote sense). Vì vậy, các tín hiệu \texttt{DAC}, \texttt{EN}, \texttt{PWM} trong schematic này được hiểu là giao diện điều khiển ở cấp hệ thống của bo mạch (hoặc qua mạch ghép trung gian), dùng để MCU điều khiển nhánh HV một cách chủ động và an toàn.
...
\begin{figure}[H]
    ...
    \caption{Schematic khối điều khiển SOLENOID}
