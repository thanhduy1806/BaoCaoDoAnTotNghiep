...
Từ các mục tiêu đã nêu ở Chương 1 và kiến trúc IF--EXP--LASER-PHOTO trình bày trong Chương 2, yêu cầu của phần cứng trong đề tài không chỉ dừng ở việc vận hành từng board riêng lẻ mà phải bảo đảm toàn hệ hoạt động như một khối thống nhất, trong đó giao tiếp dữ liệu liên board là mục tiêu trung tâm.

\begin{itemize}
    \item \textbf{Yêu cầu tích hợp hệ thống:}
    Hệ thống phải được thiết kế để các board EXP, IF, LASER và PHOTO kết nối thành một cấu trúc đồng bộ.
    ...
    Người dùng gửi lệnh đến EXP, EXP điều phối hoạt động thí nghiệm, điều khiển board LASER và thu dữ liệu từ PHOTO. Bên cạnh đó còn đọc các điều kiện môi trường thí nghiệm như nhiệt độ, độ ẩm từ các cảm biến để điều khiển môi trường thí nghiệm. Từ dữ liệu thu thập được trên board PHOTO, sau khi phân tích đóng gói dữ liệu board EXP gửi dữ liệu về cho người dùng, bên cạnh đó board IF đóng vai trò là board interface của hệ thống, dùng để giao tiếp với các module phần cứng bên ngoài khác và chuyển đổi một số giao thức kết nối như SPI sang LVDS hay LVDS từ FPGA.
...
Từ ràng buộc đó, phần cứng không thể triển khai theo hướng gom toàn bộ chức năng lên một board duy nhất, cùng với yêu cầu về mục tiêu là tùy biến thí nghiệm nên phải phân tách thành các board chức năng độc lập gồm điều khiển, giao tiếp, thực hiện thí nghiệm. Cách phân tách này vừa bảo đảm khả năng bố trí theo kích thước payload, vừa tạo thuận lợi cho tổ chức đường tín hiệu, phân bố linh kiện và tích hợp cơ--điện của toàn hệ thống cũng như có thể đáp ứng với nhiều yêu cầu thí nghiệm khác nhau.
...
EXP là bộ phận điều khiển chính của toàn bộ hệ thống thí nghiệm
Trên board EXP tích hợp 2 core xử lý bao gồm core MPU (SAMA7G54) và core MCU (SAMV71). Core MCU là nơi điều khiển, thu thập, tính toán với các ngoại vi, đồng thời đảm nhận vai trò chính trong việc vận hành thí nghiệm. MCU sẽ điều khiển 2 board thí nghiệm chính bên dưới là LASER và PHOTO. Bên cạnh việc điều khiển, theo dõi và tính toán lượng năng lượng phát - thu được từ 2 board LASER và PHOTO, MCU còn thực hiện các tác vụ kiểm soát nhiệt cho thí nghiệm nói riêng và cho toàn hệ thống nói chung. Các module HEATER, TEC sẽ được điều khiển tùy theo môi trường thực tế đọc về từ các sensor như NTC, BMP390,... Ngoài ra còn một số tác vụ khác cũng được điều khiển bởi MCU. Core MCU không đủ khả năng tính toán và xử lý với khối lượng dữ liệu lớn, MCU chỉ như là một thành phần thu thập và điều khiển, điều này dẫn đến sự thiết yếu của một con chip xử lý tác vụ mạnh hơn MCU, đó là lý do sử dụng thêm một core MPU trên board. MPU như một máy tính nhỏ sẽ chịu trách nhiệm xử lý dữ liệu và giao tiếp ra người dùng. Các lệnh điều khiển hệ thống từ người dùng sẽ được gửi đến core MPU, MPU sẽ điều phối thực hiện lệnh trên chính nó hoặc ra lệnh xuống MCU nếu các lệnh liên quan đến tác vụ thí nghiệm. Khi dữ liệu được thu thập về từ MCU, MPU sẽ xử lý và gửi về người dùng hay là những board interface khác.
...
Các linh kiện laser diode là các linh kiện bán dẫn được điều khiển dựa trên cường độ dòng điện cố định, trong hệ thống phần lớn các linh kiện điện tử điều cố định điện áp còn dòng điện thì tùy tải tiêu thụ. Yêu cầu đặt ra là phải có một nguồn dòng ổn định và có thể tùy ý điều chỉnh ngõ ra theo ngõ vào, từ kiến thức của các môn học và tìm hiểu từ các thiết kế mạch nguồn dòng ( V - I converter) nhóm quyết định chọn thiết kế như hình~\ref{fig:laser-ref}.
...
\begin{itemize}
    ...
    \item Đáp ứng tần số và độ ổn định của hệ thống: Sơ đồ tham khảo sử dụng mạng bù RC tại ngõ ra của opamp giúp soát pha phản hồi của tín hiệu ngõ ra với ngõ vào. Cách thiết kế này cho phép người thiết kế tùy chỉnh các giá trị tụ điện và điện trở phản hồi để điều chỉnh lề dự trữ pha (Phase Margin), giúp triệt tiêu hiện tượng tự dao động và ổn định dòng tải.
\end{itemize}
...
Đặc tuyến mô tả quá trình tăng dần ngõ vào và theo dõi ngõ ra. Trong hình thấy được tại ngưỡng ngõ vào khoảng 0.3V thì ngõ ra mới bắt đầu có dòng. Theo công thức (3.14) ở phần tính toán lý thuyết $V_{IN,ngưỡng}=\frac{V^+}{2}\left(1-\frac{R_1}{R_2}\right)$, với $R_1=95k\Omega , R_2= 100k\Omega, V_{IN,ngưỡng}= 0.3$ xấp xỉ với mô phỏng. Và sau 0.3 dòng điện sẽ tăng tuyến tính theo điện áp.
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối swich laser internal}
...
Tầng thứ hai sử dụng dụng bộ lọc thông thấp RC để lọc các tín hiệu nhiễu tần số cao, đảm bảo tín hiệu đọc về ADC trong ngưỡng quy định. Sau khi qua bộ lọc RC tín hiệu qua một tầng opamp buffer để điều tiết quá trình chuyển mạch và cách ly trở kháng hai phía của mạch.
