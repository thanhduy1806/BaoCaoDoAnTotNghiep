...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{picture/so-do-tong-quat.png}
...
\begin{itemize}
    ...
    IF là tầng \textit{interface}, có nhiệm vụ chuẩn hóa kết nối, tổ chức đường tín hiệu và hỗ trợ mở rộng ngoại vi. Khối này giúp tách lớp điều khiển chính khỏi các ràng buộc kết nối phần cứng, nhờ đó hệ thống dễ tích hợp thêm module mới, đồng thời giảm độ phức tạp khi thay đổi cấu hình thử nghiệm.
...
\begin{itemize}
    ...
    \item \textbf{Khối 2 eMMC:} bộ nhớ lưu trữ dung lượng lớn cho chương trình, dữ liệu hệ thống và nhật ký vận hành.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{picture/exp/block-exp-detail-1.png}\\[1em]
    \includegraphics[width=1\textwidth]{picture/exp/block-exp-detail-2.png}
    \caption{Sơ đồ kết nối chi tiết board EXP}
    \label{fig:so-do-ket-noi-chi-tiet-exp}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{picture/if/block-if.png}
...
\begin{itemize}
    ...
    \item \textbf{Khối FPGA:} là phần tử xử lý logic khả trình, phối hợp với SAMA7G54 qua giao tiếp UART và SPI, đồng thời đảm nhiệm xử lý các tín hiệu tốc độ cao và các đường LVDS trên board IF.
    ...
    \item \textbf{Khối STX LVDS, CON MAIN LVDS và CON SUB LVDS:} tạo tuyến truyền nhận tín hiệu LVDS giữa FPGA với các đầu nối ngoài, phục vụ các giao tiếp vi sai tốc độ cao và tăng khả năng chống nhiễu.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{picture/if/block-if-detail-1.png}\\[1em]
    \makebox[\textwidth][l]{\includegraphics[width=0.7\textwidth]{picture/if/block-if-detail-2.png}}
    \caption{Sơ đồ kết nối chi tiết board IF}
    \label{fig:so-do-ket-noi-chi-tiet-if}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Khối CAMERA SENSOR:} thu nhận hình ảnh hoặc dữ liệu quang học bổ sung trong buồng thí nghiệm. Khối này xuất dữ liệu ảnh qua giao tiếp \textit{MIPI} và nhận cấu hình điều khiển qua bus \textit{I2C}.

    \item \textbf{Khối MIPI SWITCH và I2C MULTIPLEXER:} các tín hiệu MIPI và I2C được kết nối từ EXP thông qua connector, do số lượn tín hiệu có hạn nhưng cần 4 camera sensor nên phải qua khối MIPI switch và I2C multiplexer để phân chia kênh điều khiển.
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối MRAM kết nối MCU}
...
\begin{table}[H]
    ...
    \hline
    Bay hơi khi mất điện & Không (non-volatile)& Có (volatile), dữ liệu được giữ nguyên & Không (non-volatile)\\
    \hline
    \end{tabularx}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối Debug}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối chuyển mạch USB}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối USB host only}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý connector BMP390}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối MPD200A}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối driver HD4}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý connector HD4}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối giao tiếp flow sensor}
    \label{fig:exp-sche-flow-ic}
\end{figure}

Cảm biến lưu lượng (flow sensor) là thiết bị dùng để đo tốc độ hoặc khối lượng của chất lỏng hoặc khí chảy qua một ống dẫn. IC được kéo ra connector và được điều khiển bởi tín hiệu I2C. IC flow kết hợp với IC bơm để tạo ra một hệ phản hồi kín, trong đó một thành phần đóng vai trò điều khiển công suất bơm lưu và thành phần còn lại đóng vai trò theo dõi lưu lượng bơm để trả về dữ liệu cho MCU xử lý, cân chỉnh IC bơm cũng như lưu trữ thông tin bơm. 

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{picture/exp/solenoid-ic.png}
    \caption{Sơ đồ nguyên lý khối điều khiển solenoid thứ nhất}
...
\begin{figure}[H]
    ...
    \caption{Sơ đồ nguyên lý khối điều khiển solenoid thứ 2}
...
\begin{itemize}
    ...
    \item Cấu trúc high-side với MOSFET kênh P: so với các cấu trúc nguồn dòng kiểu low-side hoặc nguồn dòng dùng transistor lưỡng cực (BJT), thiết kế này sở hữu các ưu điểm kỹ thuật nổi bật. Tối ưu hóa quản lý ground, việc đặt MOSFET ở phía nguồn (high-side) cho phép tải được nối trực tiếp xuống ground. Đây là yếu tố quan trọng trong thiết kế hệ thống vệ tinh, giúp giảm nhiễu điện từ (EMI), đơn giản hóa việc đi dây (routing) và hỗ trợ tản nhiệt trực tiếp cho linh kiện tải qua mặt phẳng ground của PCB. Điều khiển bằng điện áp với trở kháng cao, MOSFET là linh kiện điều khiển bằng điện áp, do đó dòng điện tiêu thụ tại cực G là rất nhỏ. Điều này giúp Op-amp hoạt động ổn định hơn, không bị quá tải dòng điện ngõ ra và giảm thiểu sai số do dòng phân cực gây ra so với khi dùng BJT.
...
Tính toán công thức ngưỡng $V_{IN}$ bật FET
\begin{itemize}
    ...
    V_S \approx V^+
    \Rightarrow
    V_-=\frac{V_{IN}\times R_2+V^+\times R_1}{R_1+R_2}
    \end{equation*}

    \item Áp dụng điều kiện ngưỡng $V_-=V_+=\dfrac{V^+}{2}$:
    \begin{equation*}
    \frac{V_{IN}\times R_2+V^+\times R_1}{R_1+R_2}=\frac{V^+}{2}
    \end{equation*}
    \begin{equation*}
    2\times\left(V_{IN}\times R_2+V^+\times R_1\right)=V^+\times(R_1+R_2)
    \end{equation*}
    \begin{equation*}
    2\times V_{IN}\times R_2=V^+\times(R_2-R_1)
    \end{equation*}
    \begin{equation}
    \boxed{V_{IN,ngưỡng}=\frac{V^+}{2}\left(1-\frac{R_1}{R_2}\right)}
    \end{equation}
\end{itemize}
...
Vai trò của tụ điện hồi tiếp ở ngõ ra: Tụ điện phản hồi $C$ đặt song song với trở phản hồi tạo nên một vòng lặp phụ hoạt động dựa trên sự phân tách tần số tín hiệu:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{picture/photo/photo-cam.png}
    \caption{Sơ đồ nguyên lý khối cảm biến ảnh của board PHOTO}
    \label{fig:photo-cam}
\end{figure}

AR2020 là cảm biến ảnh CMOS số độ phân giải 20 MP của onsemi, dùng để thu nhận ảnh quang học và chuyển trực tiếp thành luồng dữ liệu số tốc độ cao cho khối xử lý phía sau. Theo datasheet, cảm biến này dùng ma trận điểm ảnh BSI 1.4 $\mu$m, mạch đọc rolling shutter và các khối nội tích hợp như khuếch đại, lấy mẫu, số hóa và đóng gói dữ liệu ảnh; việc cấu hình các thanh ghi được thực hiện qua giao tiếp I2C Fast-mode Plus, còn ngõ ra dữ liệu là MIPI D-PHY 2$\times$4 lane cùng cặp clock vi sai. Ưu điểm của AR2020 so với nhiều cảm biến cùng lớp là độ phân giải cao ở mức công suất thấp, hỗ trợ LI-HDR/eDR, đáp ứng NIR tốt tại 850 nm và 940 nm, đồng thời có các tính năng SmartROI, Wake-on-Motion và context switching rất phù hợp cho hệ thống quang điện đa chế độ.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{picture/photo/photo-mipi.png}
    \caption{Sơ đồ nguyên lý khối chuyển mạch MIPI của board PHOTO}
    \label{fig:photo-mipi}
\end{figure}

PI3WVR626 là mạch chuyển mạch analog tốc độ cao chuyên cho tín hiệu MIPI, dùng để chọn một trong hai nguồn camera đưa về bus CSI/DSI chung. IC hoạt động như cụm công tắc SPDT cho các cặp vi sai clock và data; chân điều khiển số \texttt{SEL} quyết định nhánh nào được nối qua, còn chân \texttt{*OE} dùng để cho phép hoặc cô lập đường truyền. Ngõ vào và ngõ ra đều là các cặp tín hiệu MIPI D-PHY/C-PHY mức thấp, không phải giao thức điều khiển thanh ghi. Ưu điểm của PI3WVR626 là điện trở dẫn thấp, độ lệch kênh nhỏ, băng thông cao và tối ưu riêng cho MIPI nên suy hao, méo mắt tín hiệu và xuyên nhiễu thường tốt hơn cách ghép bằng công tắc analog thông dụng.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{picture/photo/photo-i2c-switching.png}
    \caption{Sơ đồ nguyên lý khối chuyển mạch I2C của board PHOTO}
    \label{fig:photo-i2c-switching}
\end{figure}

PCA9544A là bộ ghép kênh I2C 1-trong-4 có logic ngắt, dùng để tách một bus I2C chính ra bốn nhánh cảm biến độc lập nhằm tránh xung đột địa chỉ và cho phép chọn riêng từng camera. IC nhận lệnh điều khiển qua bus I2C/SMBus ở cặp \texttt{SDA}, \texttt{SCL}; thanh ghi điều khiển nội sẽ đóng một cặp pass-gate tương ứng để nối bus chính tới kênh \texttt{SDx/SCx} mong muốn, đồng thời gom các ngắt \texttt{INT0...INT3} thành một ngắt tổng \texttt{INT}. Ngõ vào điều khiển là I2C, còn ngõ ra cũng vẫn là các đường I2C đã được chọn kênh. Ưu điểm của PCA9544A là hỗ trợ dịch mức điện áp giữa nhiều miền nguồn, có ngõ ngắt riêng cho từng nhánh và cấu hình đơn giản hơn giải pháp dùng nhiều FET/discrete switch rời.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{picture/photo/photo-i2c-io.png}
    \caption{Sơ đồ nguyên lý khối mở rộng I/O điều khiển camera}
    \label{fig:photo-i2c-io}
\end{figure}

TCA6416A là bộ mở rộng 16 GPIO điều khiển qua I2C/SMBus, được dùng để tạo thêm các chân điều khiển như \texttt{GPIO0}, \texttt{nOFF}, \texttt{SEN\_SEL} hay \texttt{CLK\_nENA} cho nhiều camera từ một bus nối tiếp hai dây. Bên trong IC là các thanh ghi input, output, polarity inversion và configuration; vi điều khiển ghi/đọc các thanh ghi này qua \texttt{SDA}, \texttt{SCL}, từ đó biến các chân \texttt{P00...P17} thành ngõ vào hoặc ngõ ra số tùy nhu cầu. Vì vậy ngõ vào điều khiển là I2C/SMBus, còn ngõ ra là các tín hiệu số mức CMOS/TTL theo miền nguồn \texttt{VCCP}. Ưu điểm của TCA6416A là vừa mở rộng I/O vừa hỗ trợ dịch mức giữa \texttt{VCCI} và \texttt{VCCP}, có chân \texttt{INT}, \texttt{RESET}, dòng chờ thấp và gọn hơn đáng kể so với dùng vi điều khiển phụ chỉ để tạo GPIO.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{picture/photo/photo-osc.png}
    \caption{Sơ đồ nguyên lý khối dao động và logic điều phối xung clock}
    \label{fig:photo-osc}
\end{figure}

DSC1001DL5-027.0000T là dao động MEMS CMOS tần số 27 MHz, dùng làm nguồn clock chuẩn cho hệ camera. IC tích hợp phần tử cộng hưởng MEMS, mạch PLL bù và tầng đệm đầu ra trong cùng một gói; khi cấp \texttt{VDD}, chân \texttt{OUT} phát ra xung clock CMOS còn \texttt{STANDBY\#} cho phép đưa linh kiện về chế độ tiết kiệm điện. Ngõ vào là nguồn nuôi và tín hiệu enable mức số, ngõ ra là xung clock CMOS đơn cực. Ưu điểm của DSC1001 là chịu sốc rung tốt hơn tinh thể thạch anh, khởi động nhanh, ổn định tần số tốt và dễ thay thế trực tiếp cho nhiều footprint oscillator chuẩn.

NL7SZ97DT2G-Q là cổng logic đa chức năng cấu hình được, ở đây được dùng như khối gating/phân phối clock cho từng nhánh cảm biến thông qua tín hiệu \texttt{CLK\_nENA}. IC nhận các tín hiệu số tại các chân \texttt{A}, \texttt{B}, \texttt{C} và tạo ra đầu ra \texttt{Y} theo hàm logic được cấu hình bằng cách nối mức các chân điều khiển; trong sơ đồ này đầu vào và đầu ra đều là tín hiệu số CMOS mức 1.8 V. Ưu điểm của NL7SZ97 là một IC nhỏ có thể thay cho nhiều loại cổng đơn như AND, OR, NAND, NOR, buffer hay inverter, giúp giảm số lượng mã linh kiện và tối ưu diện tích mạch so với dùng từng cổng cố định riêng lẻ.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{picture/photo/photo-ldos.png}
    \caption{Sơ đồ nguyên lý khối LDO cấp nguồn cho board PHOTO}
    \label{fig:photo-ldos}
\end{figure}

TLV75733PDYDR là LDO 3.3 V dòng tối đa 1 A, dùng để hạ áp từ \texttt{+5V\_CAM} xuống \texttt{+3.3V} cho các khối số và tham chiếu. IC làm việc theo nguyên lý điều khiển phần tử pass nội với vòng hồi tiếp giữ ổn định điện áp ra; ngõ vào là \texttt{VIN}, tín hiệu số \texttt{EN}, còn ngõ ra là \texttt{VOUT} analog DC. Ưu điểm của TLV75733 là dòng cấp lớn, sai số thấp và dòng tĩnh nhỏ, phù hợp hơn các LDO phổ thông khi cần vừa gọn vừa đủ dòng cho nhiều tải số.

TPS7A2024PDBVR là LDO 2.4 V nhiễu thấp, được dùng để tạo rail \texttt{+2.4V} cho phần giao tiếp ảnh. Ngõ vào là nguồn DC \texttt{VIN} và chân \texttt{EN}, ngõ ra là nguồn DC ổn áp \texttt{VOUT}; IC dùng transistor pass và bộ khuếch đại lỗi nội để duy trì điện áp ổn định với PSRR cao. So với nhiều LDO cùng mức áp, dòng tĩnh và nhiễu đầu ra của họ TPS7A20 thường thấp hơn, thích hợp cho tải nhạy nhiễu như cảm biến ảnh và giao tiếp tốc độ cao.

TLV75528PDBVR là LDO 2.8 V cho rail analog/pixel của cảm biến. Cũng như các LDO khác, IC nhận nguồn DC tại \texttt{VIN}, chân bật/tắt \texttt{EN} và xuất điện áp DC đã ổn áp tại \texttt{VOUT}. Điểm mạnh của TLV75528 là PSRR khá tốt, khởi động mềm và yêu cầu tụ ngoài nhỏ, nên dễ tích hợp cho các rail analog gần cảm biến.

TLV75518PDBVR là LDO 1.8 V cấp cho logic I/O, clock hoặc các khối MIPI phụ trợ. Ngõ vào/ra là nguồn DC cùng chân enable số; nguyên lý vẫn là ổn áp tuyến tính với phần tử pass nội và mạch bảo vệ dòng/nhiệt. Ưu điểm của mã này là tiêu thụ tĩnh thấp, kích thước nhỏ và đáp ứng tốt cho các miền 1.8 V rất phổ biến trong camera module.

NCP139AFCT105T2G là VLDO 1.05 V dòng cao có chân \texttt{VBIAS} riêng, dùng để tạo rail lõi số điện áp thấp cho cảm biến. IC nhận \texttt{VIN}, \texttt{VBIAS}, \texttt{EN} và phản hồi \texttt{SNS/FB}, sau đó điều khiển NMOS pass để tạo \texttt{VOUT} sụt áp rất thấp; vì có chân bias riêng nên mạch điều khiển nội không phải lấy toàn bộ năng lượng từ đường công suất chính. Ưu điểm của NCP139 so với LDO lõi thông thường là dropout rất thấp ở dòng tải lớn, độ chính xác tốt và đặc biệt phù hợp cho rail core điện áp thấp của cảm biến/SoC.
...
ADG1414BRUZ là mạch chuyển mạch analog 8 kênh SPST điều khiển nối tiếp, dùng để chọn photodiode nào được đưa vào chuỗi khuếch đại dòng-quang phía sau. IC nhận dữ liệu cấu hình qua giao tiếp SPI tương thích \texttt{SCLK}, \texttt{DIN}, \texttt{SYNC/CS}; sau khi ghi thanh ghi 8 bit, từng công tắc \texttt{S1...S8} sẽ đóng hoặc ngắt để nối các nút photodiode tương ứng sang các chân \texttt{D1...D8}. Như vậy ngõ vào điều khiển là giao tiếp nối tiếp kiểu SPI, còn đường tín hiệu qua công tắc là tín hiệu analog hai chiều. Ưu điểm của ADG1414 là điện trở dẫn thấp, độ phẳng $R_{ON}$ tốt, chịu dải tín hiệu rộng tới rail nguồn và hỗ trợ daisy-chain, nên phù hợp hơn các analog switch đời cũ khi cần đa kênh, độ méo thấp và điều khiển gọn.
